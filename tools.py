...
Contents:
    - `read_memory`                  ->  read from persistent MEMORY.md file
    - `update_memory`                ->  append a new entry to the persistent MEMORY.md file
    ...
    - `fetch_webpage`                ->  fetch and return full webpage text content
    - `generate_research_report`     ->  generate a structured markdown research report
"""
...
def web_search(query: str, max_results: int = 10) -> str:
    ...
    Each result includes a title, URL, and text snippet.  Use this to
    discover sources and gather initial information on a topic.  To get
    the full content of a promising result, follow up with `fetch_webpage`.

    Args:
        query:       The search query string (e.g. 'latest advances in fusion energy 2025').
        max_results: Maximum number of results to return (1-20, default 10).

    Returns:
        Numbered search results with title, URL, and snippet for each.
    """
...
def fetch_webpage(url: str, max_chars: int = 12000) -> str:
    ...
    Args:
        url:       The full URL of the webpage to fetch (e.g. 'https://…').
        max_chars: Maximum characters to return (default 12000).

    Returns:
        The page title and body text in Markdown format, or an error message.
    """
    import requests
...
def generate_research_report(
    ...
    Args:
        topic:       The research question or topic (e.g. 'Recent advances in
                     photonic computing').
        findings:    The full synthesized research text produced by the agent.
                     Can contain markdown formatting, bullet points, headings,
                     and inline URLs.  URLs are automatically extracted and
                     collected into a Sources section.
        output_path: Where to save the report.  If empty, defaults to
                     'reports/research_report_YYYY-MM-DD.md'.

    Returns:
        Confirmation message with the path to the saved report.
    """
    import re
